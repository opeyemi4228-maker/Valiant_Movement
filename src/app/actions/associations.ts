...
import { chapterView, type ChapterPath, type ChapterView } from "@/lib/associations-db";
...
export async function getChapters(path: ChapterPath): Promise<ChaptersResult> {
  ...
  try {
    const view = await withRetry(() => chapterView(clamped));
